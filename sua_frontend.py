...
if "last_category" not in st.session_state:
    st.session_state.last_category = "전체"    
...
if menu == "💬 분석 챗봇":
    ...
    # 카테고리 버튼 추가
    # --------------------------
    categories = ["전체", "제조업", "IT", "유통", "금융", "정책"]

    category_prompts = {
        "전체": "전반적인 국내 취업 시장 동향을 알려줘",
        "제조업": "최근 제조업 취업 및 채용 동향을 분석해줘",
        "IT": "최근 IT 업계의 채용 및 감원 동향을 알려줘",
        "유통": "최근 유통업 채용 트렌드를 알려줘",
        "금융": "최근 금융업 채용 변화 흐름을 알려줘",
        "정책": "최근 고용 정책의 핵심 내용을 알려줘",
    }

    selected_category = st.segmented_control(
        "분석 카테고리 선택",
        options=categories,
        default=st.session_state.last_category
    )
